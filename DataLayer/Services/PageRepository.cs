...
namespace DataLayer
{
    public class PageRepository : IPageRespository
    {
        ...
        {
            return db.Pages;     //return all the pages
        }
        ...
        {
            return db.Pages.Where(p => p.Title.Contains(search) || p.ShortDescription.Contains(search) || p.Tags.Contains(search) || p.PageText.Contains(search)).Distinct();
        } 
    }
}
